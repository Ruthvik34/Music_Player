...
class SongAdapter(private val listener: OnItemClickListener) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.ivPlayHint.setOnClickListener {
            val pos = holder.bindingAdapterPosition
            if (pos != RecyclerView.NO_POSITION) listener.onPlayClick(pos)
        }
    ...
    private fun updateLikeButton(holder: ViewHolder, song: Song) {
        ...
        holder.binding.ivLike.imageTintList = ColorStateList.valueOf(
            ContextCompat.getColor(
                context,
                if (liked) R.color.accent_secondary else R.color.text_hint
            )
        )
